...
"""Contains a tool to reformat static imports in `./src/data_analysis_with_agent/__init__.py`."""
...
def check_static_imports(update: bool, package: str) -> NoReturn:
    """Check all imports are made twice (1 in lazy-loading and 1 in static checks).

    For more explanations, see `./src/data_analysis_with_agent/__init__.py`.
    This script is used in the `make style` and `make quality` checks.
    """
    module_name = "data_analysis_with_agent"
    if package:
        module_name = module_name + "." + package
    module = importlib.import_module(module_name)
    _SUBMOD_ATTRS = getattr(module, "_SUBMOD_ATTRS")

    INIT_FILE_PATH = Path(__file__).parents[1] / "src" / "data_analysis_with_agent" / package / "__init__.py"
    ...
    if SUBMOD_ATTRS_PATTERN.search(init_content_before_static_checks) is None:
        print("Error: _SUBMOD_ATTRS dictionary definition not found in `./src/data_analysis_with_agent/__init__.py`.")
        exit(1)
    ...
    if init_content != expected_init_content:
        if update:
            ...
            print(
                "✅ Imports have been updated in `./src/data_analysis_with_agent/__init__.py`."
            ...
            print(
                ...
                " `./src/data_analysis_with_agent/__init__.py`.\n   It is most likely that you"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--update",
        action="store_true",
        help="Whether to fix `./src/data_analysis_with_agent/__init__.py` if a change is detected.",
    )
    parser.add_argument("--package", type=str, default="", help="App startup port")
    args = parser.parse_args()

    check_static_imports(update=args.update, package=args.package)
